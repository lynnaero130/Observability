...
hold on
plot(time(1:length(xt(2,:))),xt(2,:),'m--', 'linewidth', 1)
plot(time(1:length(xt(3,:))),xt(3,:),'b--', 'linewidth', 1)

plot(time(1:length(xt(1,:))),gtd(1,1:length(xt(1,:))),'r-', 'linewidth', 1)
plot(time(1:length(xt(2,:))),gtd(2,1:length(xt(2,:))),'m-', 'linewidth', 1)
...
legend({'x\_hat','y\_hat','z\_hat','x_g','y_g','z_g'},'FontSize',12,'NumColumns',3)
